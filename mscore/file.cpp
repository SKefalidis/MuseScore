...
{
    QString allExt = "*.mscz *.mscx *.mxl *.musicxml *.xml *.mid *.midi *.kar *.md *.mgu *.sgu *.cap *.capx *.ove *.scw *.bww *.gtp *.gp3 *.gp4 *.gp5 *.gpx *.gp *.ptb *.mscz, *.mscx, *.msca *.album";
#ifdef AVSOMR
...
void MuseScore::openAlbum(const QString& fn)
{
    ...
    if (!f.open(QIODevice::ReadOnly)) {
        qDebug() << "Could not open filestream to open album" << endl;
        return;
    }
    ...
    while (reader.readNextStartElement()) {
        ...
        if (tag == "name") {
            albumManager->albumTitleEdit->setText(reader.readElementText());
        } else if (tag == "Score") {
            readAlbumScore(reader);
        }
    }
}
...
void MuseScore::readAlbumScore(XmlReader& reader)
{
    bool enabled { false };
    while (reader.readNextStartElement()) {
        const QStringRef& tag(reader.name());
        if (tag == "name") {
            ...
            openScore(reader.readElementText(), true);
        } else if (tag == "enabled") {
    ...
    albumManager->addScore(currentScoreView()->score()->masterScore(), enabled);
}
...
//            addRecentScore(score); add recent album!
//            writeSessionFile(false);

    QFile f(fileBaseName);
    if (!f.open(QIODevice::WriteOnly)) {
        qDebug() << "Could not open filestream to save album" << endl;
        return false;
    }

    XmlWriter xml(nullptr, &f);
    xml.header();
    xml.stag(QStringLiteral("museScore version=\"" MSC_VERSION "\" full-version=\"%1\"").arg(fullVersion()));
    xml.stag("Album");
    xml.tag("name", albumManager->albumTitleEdit->text());
    albumManager->saveCurrentAlbum(xml);
    xml.etag();
    xml.etag();
    f.close();
    return true;
